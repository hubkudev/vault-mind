<script lang="ts">
    ...
    import type { IVault } from "../../types/vault";
    ...
    let {
        user,
        vault,
        contentInput = $bindable<string>(),
        fileInput = $bindable<string>(),
        typeInput = $bindable<string>(),
    ...
    const knowledgeTrigger = $derived(
        knowledgeTypes.find((f) => f.value === typeInput)?.label ??
...
<nav class="w-full flex items-center justify-between px-12 py-2 shadow">
    <div class="flex items-center gap-4">
        ...
        <p class="font-bold text-lg">{vault.name}</p>
    </div>
    ...
    <div class="flex items-center gap-6">
        <div>
            <Dialog.Root bind:open={dialogOpen}>
                ...
                <Dialog.Content>
                    <Dialog.Header>
                        <Dialog.Title>Create New Knowledge</Dialog.Title>
                        <Dialog.Description>
                    ...
                    <div class="grid gap-4 py-2 mb-2">
                        <Select.Root type="single" bind:value={typeInput}>
                            <Select.Trigger>{knowledgeTrigger}</Select.Trigger>
                            <Select.Content>
                                {#each knowledgeTypes as types}
                                    <Select.Item
                                        value={types.value}
                                        label={types.label}
                                        >{types.label}</Select.Item
                                    >
                                {/each}
                            </Select.Content>
                        </Select.Root>
                        ...
                        {#if typeInput === "text"}
                            <div>
                                <Label for="description" class="text-right"
                                    >Content</Label
                                >
                                <Textarea rows={20} bind:value={contentInput} />
                            </div>
                        {/if}
                    ...
                    {/if}
                    <Dialog.Footer>
                        <Button onclick={onVaultSubmit}>Save Knowledge</Button>
                    </Dialog.Footer>
                </Dialog.Content>
            </Dialog.Root>
        </div>
